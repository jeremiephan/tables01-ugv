...
(function () {
  ...
  // v4 toggles : default 'My doctrine only' filter ON, 'Show partnerships' OFF
  if (typeof state.showAllActors !== 'boolean') state.showAllActors = false;
  if (typeof state.showPartnerships !== 'boolean') state.showPartnerships = false;
  ...
  function renderMap() {
    ...
    // Defs — hatch patterns for contested (one per country, since visual encoding is by country in v4)
    const defs = el('defs');
    for (const k in D.COUNTRY_COLORS) {
      const color = D.COUNTRY_COLORS[k];
    ...
    // v4 filter: if showAllActors === false AND thesis is not D (no bet) or E (mixed),
    // display only actors matching the user's thesis doctrine
    const filterDoctrine = !state.showAllActors &&
                           state.thesis &&
                           state.thesis !== 'D' &&
                           state.thesis !== 'E';

    for (const s of D.STARTUPS) {
      // Apply doctrine filter (v4)
      if (filterDoctrine && s.doctrine !== state.thesis) continue;

      const pos = startupPosition(s);
      const r = D.FUNDING_RADIUS[s.fundingScale] || 5;
      const color = D.COUNTRY_COLORS[s.country] || '#a0a8bc';
  ...
  function showTooltip(s, evt) {
    if (!tooltip) return;
    tooltip.innerHTML =
      `<div class="tt-eyebrow">${s.code} · ${s.country} · ${D.COUNTRIES[s.country] ? D.COUNTRIES[s.country].name : ''}</div>` +
  ...
  function renderPopover() {
    ...
    popover.innerHTML = `
      ...
      <div class="po-eyebrow">${s.code} · ${c ? c.name : s.country}</div>
      ...
      <div class="po-fit">
        <span class="po-fit-k">Thesis fit</span>
  ...
  function renderRecap() {
      ...
      if (sel.length === 0) {
        ...
        const rows = sel.map(s => `
          <div class="rc-row" data-code="${s.code}">
            ...
            <span class="rc-row-country" style="color:${D.COUNTRY_COLORS[s.country] || '#a0a8bc'}">${s.country}</span>
    ...
    // Country mix (replaces doctrine mix in v4 — visual encoding is by country)
    const mixEl = document.getElementById('rc-mix');
    if (mixEl) {
      const total = sel.reduce((a, b) => a + b.amount, 0) || 1;
      // Group by country, sorted by amount descending
      const countryAmounts = {};
      sel.forEach(s => {
        countryAmounts[s.country] = (countryAmounts[s.country] || 0) + s.amount;
      });
      const sortedCountries = Object.keys(countryAmounts).sort((a, b) => countryAmounts[b] - countryAmounts[a]);
      const rows = sortedCountries.map(c => {
        const sum = countryAmounts[c];
        const p = total > 0 ? (sum / total) * 100 : 0;
        const cName = D.COUNTRIES[c] ? D.COUNTRIES[c].name : c;
        return `
          <div class="rc-mix-row">
            <span class="rc-mix-k">${c} · ${cName}</span>
            <span class="rc-mix-bar"><span class="rc-mix-fill" style="width:${p.toFixed(1)}%; background:${D.COUNTRY_COLORS[c] || '#a0a8bc'}"></span></span>
            <span class="rc-mix-v">${Math.round(p)}%</span>
          </div>
        `;
      }).join('');
      mixEl.innerHTML = `<div class="rc-section-h">Country mix</div>${rows || '<div class="rc-empty">No positions yet.</div>'}`;
    }
  ...
  function updateFilterMessage() {
    const msgEl = document.getElementById('step3-filter-message');
    if (!msgEl) return;
    if (state.showAllActors || state.thesis === 'D' || state.thesis === 'E') {
      const total = D.STARTUPS.length;
      msgEl.innerHTML = `<span class="s3fm-strong">All ${total} actors</span> shown across the dataset.`;
    } else {
      const matching = D.STARTUPS.filter(s => s.doctrine === state.thesis).length;
      const thesis = D.THESES[state.thesis];
      const thesisLabel = thesis ? thesis.label : state.thesis;
      msgEl.innerHTML = `Showing <span class="s3fm-strong">${matching} actors</span> matching your thesis <span class="s3fm-thesis">${thesisLabel}</span>. Click toggle below to see all ${D.STARTUPS.length}.`;
    }
  }

  function wireToggles() {
    // Toggle: Show all actors / My doctrine only
    const toggleAll = document.getElementById('toggle-show-all');
    if (toggleAll) {
      // Reflect current state on load
      toggleAll.classList.toggle('is-active', state.showAllActors);
      toggleAll.setAttribute('aria-pressed', state.showAllActors ? 'true' : 'false');
      const labelEl0 = toggleAll.querySelector('.toggle-label');
      if (labelEl0) labelEl0.textContent = state.showAllActors ? 'All 39 actors' : 'My doctrine only';
      toggleAll.addEventListener('click', () => {
        state.showAllActors = !state.showAllActors;
        saveState();
        toggleAll.classList.toggle('is-active', state.showAllActors);
        toggleAll.setAttribute('aria-pressed', state.showAllActors ? 'true' : 'false');
        const labelEl = toggleAll.querySelector('.toggle-label');
        if (labelEl) labelEl.textContent = state.showAllActors ? 'All 39 actors' : 'My doctrine only';
        renderMap();
        updateFilterMessage();
      });
    }

    // Toggle: Show partnerships
    const togglePartnerships = document.getElementById('toggle-partnerships');
    if (togglePartnerships) {
      togglePartnerships.classList.toggle('is-active', state.showPartnerships);
      togglePartnerships.setAttribute('aria-pressed', state.showPartnerships ? 'true' : 'false');
      togglePartnerships.addEventListener('click', () => {
        state.showPartnerships = !state.showPartnerships;
        saveState();
        togglePartnerships.classList.toggle('is-active', state.showPartnerships);
        togglePartnerships.setAttribute('aria-pressed', state.showPartnerships ? 'true' : 'false');
        renderMap();
